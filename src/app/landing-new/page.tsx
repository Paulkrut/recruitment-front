...
import { Box, AppBar, Toolbar, IconButton, Menu, MenuItem, Button, Typography, Tooltip } from "@mui/material";
    ...
    <Box sx={{ bgcolor: '#fafafa', minHeight: '100vh' }}>
      ...
      <AppBar position="sticky" elevation={0} sx={{ bgcolor: '#fafafa', borderBottom: '1px solid #e5e5e5' }}>
        <Box sx={{ maxWidth: 'lg', width: '100%', mx: 'auto', px: 3 }}>
          <Toolbar disableGutters sx={{ py: 1 }}>
            ...
            <Box sx={{ display: { xs: 'none', md: 'flex' }, gap: 4, mr: 4 }}>
              {menuItems.map((item) => (
                <Link key={item.label} href={item.href} style={{ textDecoration: 'none', color: '#666', fontSize: '0.9rem', fontWeight: 500 }}>
                  {item.label}
                </Link>
              ))}
            </Box>
        ...
        <Menu anchorEl={mobileMenuAnchor} open={Boolean(mobileMenuAnchor)} onClose={() => setMobileMenuAnchor(null)}>
          {menuItems.map((item) => (
            <MenuItem key={item.label} component={Link} href={item.href} onClick={() => setMobileMenuAnchor(null)}>
              {item.label}
            </MenuItem>
          ))}
        </Menu>
      ...
      {/* AI Interview Section - Ядро платформы */}
      <AiInterviewSection />

      {/* Full Features Section - Полный функционал */}
      <FullFeaturesSection />
